"""
Plot stationary resonances on full-range geophone spectrograms
"""
...
parser = ArgumentParser(description="Plot stationary resonances on full-range geophone spectrograms")

parser.add_argument("--stations", type=str, nargs="+", help="Station list")
parser.add_argument("--starttime", type=Timestamp, help="Start time")
parser.add_argument("--endtime", type=Timestamp, help="End time")
parser.add_argument("--time_to_mark", type=Timestamp, help="Time window to mark")
...
time_to_mark = args.time_to_mark
min_db = args.min_db
max_db = args.max_db
...
major_datetime_tick_spacing = "3h"
num_minor_datetime_ticks = 3
...
harmo_arrow_length = Timedelta(minutes = 15)
...
for station in stations:
    ...
    stream_psd = read_geo_stft(inpath, starttime = starttime, endtime = endtime, min_freq = min_freq, max_freq = max_freq, psd_only = True)
...
for i_sta, station in enumerate(stations):
    ...
    mappable = ax.pcolormesh(time_ax, freq_ax, psd_mat, cmap = cmap, vmin = min_db, vmax = max_db)
...
# Save the figure
###
figname = f"stationary_resonance_on_full_range_geo_spectrograms.png"
