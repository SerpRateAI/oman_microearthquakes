# Plot the tremor waveform and spectrum of a station used for the presentation at Tulane University
...
from utils_plot import format_freq_xlabels,  format_db_ylabels, save_figure, format_vel_ylabels, format_rel_time_xlabels
from utils_plot import component2label
### Inputs ###
# Command line arguments
parser = ArgumentParser(description="Input parameters for plotting the tremor waveform and spectrum of a station used for the presentation at Tulane University.")
parser.add_argument("--component", type=str, help="Component to plot.", default="Z")
parser.add_argument("--station", type=str, help="Station whose spectrum will be plotted.")
parser.add_argument("--starttime", type=str, help="Time window for the spectrum.")
...
parser.add_argument("--height_frac_spec", type=float, default=0.7, help="Fraction of the height of the figure used for the spectrum.")
parser.add_argument("--hspace", type=float, default=0.1, help="Height of the space between the waveform and the spectrum.")
...
parser.add_argument("--axis_label_size", type=float, default=14.0, help="Font size of the axis labels.")
parser.add_argument("--tick_label_size", type=float, default=12.0, help="Font size of the tick labels.")
parser.add_argument("--title_size", type=float, default=16.0, help="Font size of the title.")
...
height_frac_spec = args.height_frac_spec
hspace = args.hspace
...
stream = read_and_process_windowed_geo_waveforms(stations=station, starttime=starttime, dur=duration, components=[component])
trace = stream[0]
waveform = trace.data
...
print(f"Plotting the waveform and spectrum of station {station}...")

# Generate the figure and axes
fig = figure(figsize = (fig_width, fig_height))
height_frac_waveform = 1.0 - height_frac_spec - hspace - 2 * margin_y
ax_waveform = fig.add_axes([margin_x, margin_y, 1.0 - 2 * margin_x, height_frac_waveform])
ax_spec = fig.add_axes([margin_x, margin_y + height_frac_waveform + hspace, 1.0 - 2 * margin_x, height_frac_spec])

# Plot the waveform
timeax = linspace(0, len(waveform) / sampling_rate, len(waveform))
ax_waveform.plot(timeax, waveform, color = "black", linewidth = linewidth_waveform, zorder = 2)

ax_waveform.set_xlim(timeax[0], timeax[-1])

format_rel_time_xlabels(ax_waveform, axis_label_size = axis_label_size, tick_label_size = tick_label_size, major_tick_spacing = 60, num_minor_ticks = 6)
format_vel_ylabels(ax_waveform, axis_label_size = axis_label_size, tick_label_size = tick_label_size, major_tick_spacing = 50, num_minor_ticks = 5)

ax_waveform.set_title(f"Tremor waveform, {station}, {component2label(component)}", fontsize = title_size, fontweight = "bold")

# Plot the total spectrum
ax_spec.plot(freqax, aspec, color = "black", linewidth = linewidth_spec, zorder = 2)
...
for mode_order in mode_marker_df["mode_order"].unique():
    ...
    power = interp1d(freqax, aspec)(freq_resonance)
...
if not log_scale:
    filename = "mode_spacing_labels_tulane.csv"
...
ax_spec.set_title(f"Tremor spectrum, {station}, {component2label(component)}", fontsize = title_size, fontweight = "bold")

### Save the figure ###
print("Saving the figure...")
if log_scale:
    figname = "tulane_waveform_n_spec_log_scale.png"
else:
    figname = "tulane_waveform_n_spec.png"
