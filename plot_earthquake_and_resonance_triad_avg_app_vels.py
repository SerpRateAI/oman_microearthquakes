"""
Plot the average apparent velocities of an earthquake and the resonance signals for each station triad.
"""
...
parser = ArgumentParser(description = "Plot the apparent velocities of station triads of an earthquake and a stationary resonance in a time window.")

parser.add_argument("--earthquake_id", type=str, help="The earthquake ID", default="1")
...
parser.add_argument("--min_num_obs_eq", type=int, help="The minimum number of observations of the earthquake signal", default=5)
parser.add_argument("--min_cohe_reson", type=float, help="The minimum coherence of the resonance signal", default=0.85)
parser.add_argument("--max_back_azi_std_eq", type=float, help="The maximum back azimuth standard deviation of the earthquake signal", default=20.0)
...
min_num_obs_eq = args.min_num_obs_eq
min_cohe_reson = args.min_cohe_reson
max_back_azi_std_eq = args.max_back_azi_std_eq
...
# Load the earthquake apparent velocities
print("Loading the earthquake apparent velocities...")
filename = f"earthquake_station_triad_avg_app_vels_eq{earthquake_id}_min_num_obs{min_num_obs_eq:d}_max_back_azi_std{max_back_azi_std_eq:.0f}.csv"
filepath = join(dirname_loc, filename)
vel_eq_df = read_csv(filepath)

vel_eq_df = vel_eq_df[vel_eq_df["station1"].isin(stations_to_plot) &
                      vel_eq_df["station2"].isin(stations_to_plot) &
                      vel_eq_df["station3"].isin(stations_to_plot)]
...
for i, component in enumerate(components):
    ...
    # Plot the earthquake apparent velocities
    print("Plotting the earthquake apparent velocities...")
    triads_to_plot_dicts = []
    for _, row in vel_eq_df.iterrows():
    ...
    # Add the title
    if i == 0:
        title = f"Earthquake {earthquake_id}"
...
save_figure(fig, f"earthquake_and_resonance_triad_app_vels_eq{earthquake_id}_{mode_name}.png")
