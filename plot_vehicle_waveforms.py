...
parser = ArgumentParser(description = "Plot the vehicle signals")
parser.add_argument("--occurrence", type = str, help = "Occurrence", default = "approaching")
parser.add_argument("--buffer_time", type = float, default = 5.0, help = "Buffer time in seconds")
...
parser.add_argument("--major_tick_spacing", type = str, default = "10s", help = "Major tick spacing")
parser.add_argument("--num_minor_ticks", type = int, default = 10, help = "Number of minor ticks")
...
vehicle_df = read_csv(inpath, parse_dates = ["start_time", "end_time"])
start_time_vehicle = vehicle_df.loc[vehicle_df["occurrence"] == occurrence, "start_time"].values[0]
end_time_vehicle = vehicle_df.loc[vehicle_df["occurrence"] == occurrence, "end_time"].values[0]

# Read the station coordinates
station_df = get_geophone_coords()

# Read the waveforms
start_time_plot = Timestamp(start_time_vehicle) - Timedelta(seconds = buffer_time)
end_time_plot = Timestamp(end_time_vehicle) + Timedelta(seconds = buffer_time)
...
for i, component in enumerate(geo_components):
    ...
    ax.axvline(x = start_time_vehicle, color = "crimson", linestyle = "--", linewidth = linewidth_marker)
    ax.axvline(x = end_time_vehicle, color = "crimson", linestyle = "--", linewidth = linewidth_marker)
...
# Set the suptitle
fig.suptitle(f"{occurrence.capitalize()} vehicle, {min_freq} - {max_freq} Hz", fontsize = title_size, fontweight = "bold", y = 0.95)

# Save the figure
figname = f"vehicle_waveforms_{occurrence}.png"
