...
parser.add_argument("--figwidth", type=float, help="Width of the figure", default=15.0)
parser.add_argument("--rose_a_x", type=float, help="X-coordinate of the rose diagram for Subarray A", default=0.6)
parser.add_argument("--rose_a_y", type=float, help="Y-coordinate of the rose diagram for Subarray A", default=0.4)
...
parser.add_argument("--fontsize_label_phys", type=float, help="Fontsize of the labels of the physical properties", default=10)
parser.add_argument("--fontsize_title", type=float, help="Fontsize of the titles", default=20)
parser.add_argument("--fontsize_subtitle", type=float, help="Fontsize of the titles", default=18)
...
parser.add_argument("--panel_label_size", type=float, help="Size of the panel labels", default=12)
...
fontsize_subtitle = args.fontsize_subtitle
...
# Constants
triads_to_highlight_dicts = [{"station1": "A01" , "station2": "A03", "station3": "A04"}, {"station1": "A01" , "station2": "A06", "station3": "A04"}]
...
figheight= figwidth * (1 - 2 * margin_x - wspace) * aspect_ratio_map / (1 - 2 * margin_y)
...
ax.imshow(image, extent = extent, alpha = image_alpha)

# Plot the station triads and vectors
back_azis_to_plot_dicts = []
for _, row in vel_df.iterrows():
    # Determine the component to plot with the most observations
    max_num_obs = 0
    component_plot = None
    for component in components:
        if row[f"num_obs_{component.lower()}"] > max_num_obs:
            max_num_obs = row[f"num_obs_{component.lower()}"]
    ...
    print(f"Plotting the Component {component2label(component_plot)} vectors for the station triad {station1}-{station2}-{station3}.")

    # Find the horizontal component with the smallest apparent velocity variance
    vel_app_east = row[f"avg_app_vel_east_{component_plot.lower()}"]
    vel_app_north = row[f"avg_app_vel_north_{component_plot.lower()}"]
    vel_app = row[f"avg_app_vel_{component_plot.lower()}"]
    back_azi = row[f"avg_back_azi_{component_plot.lower()}"]
    ...
    back_azis_to_plot_dicts.append({"station1": station1, "station2": station2, "station3": station3, "back_azi": back_azi_plot, "component": component_plot})

    quiver = ax.quiver(east_center, north_center, vec_east, vec_north, vec_amp, 
                    cmap=cmap, norm=norm,
...
triads_to_highlight_df = DataFrame(triads_to_highlight_dicts)
plot_station_triads(ax, linewidth = linewidth_triad, triads_to_plot = back_azis_to_plot_df, triads_to_highlight = triads_to_highlight_df)

# Plot the annotation to the highlighted station triad
ax.text(20.0, -50.0, "Fig. 9", ha = "right", va = "bottom", fontsize = 15, fontweight = "bold", color = "crimson")

# Add the axes for the rose diagram 
ax_rose_a = ax.inset_axes([rose_a_x, rose_a_y, rose_width, rose_height], projection="polar")
ax_rose_b = ax.inset_axes([rose_b_x, rose_b_y, rose_width, rose_height], projection="polar")
...
ax_rose_a.set_yticklabels([])
ax_rose_b.set_yticks(count_ticks)
ax_rose_b.set_yticklabels(count_ticks, ha = "right", va = "center")
ax_rose_b.set_rlabel_position(90)
ax_rose_b.set_yticklabels([])
ax_rose_a.set_title("Subarray A", fontsize = fontsize_subtitle, fontweight = "bold")
ax_rose_b.set_title("Subarray B", fontsize = fontsize_subtitle, fontweight = "bold")

# Add a colorbar
cax = ax.inset_axes([cbar_x, cbar_y, cbar_width, cbar_height])
cbar = add_colorbar(fig, cax, "Velocity (m s$^{-1}$)",
                    cmap=cmap, norm=norm, axis_label_size = 15, tick_label_size = 12)
...
# Add the title
ax.set_title(f"Mode {mode_order}, propagation vectors", fontsize = fontsize_title, fontweight = "bold", y = 1.02)

### Save the figure ###
print("Saving the figure...")
filename = f"jgr_propagation_vectors.png"
