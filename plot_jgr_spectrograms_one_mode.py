# Plot the spectrograms of one mode in the JGR paper
...
# Inputs
# Parse the arguments
parser = ArgumentParser(description = "Plot the spectrograms of one mode in the JGR paper")
...
parser.add_argument("--nonlinear_colormap", action = "store_true", help = "Whether make the colormap non-linear")
...
nonlinear_colormap = args.nonlinear_colormap
...
print("### Plotting the spectrograms in the JGR paper ###")
...
if nonlinear_colormap:
        # Create nonlinear sampling
        x = linspace(0, 1, 256)
        gamma = 3.0   # try 1.5–3.0
        warped_x = x**gamma

        # Build new colormap
        cmap_base = colormaps[colormap_name]
        new_colors = cmap_base(warped_x)
...
cbar.locator = MultipleLocator(major_db_spacing)
cbar.update_ticks()
...
figname = f"jgr_spectrograms_{mode_name}.{format}"
