...
parser.add_argument("--color", type = str, default = "tab:blue", help = "The color of the mode")
...
parser.add_argument("--overlap", type = float, default = 0.0, help = "The STFT overlap (0-1)")
parser.add_argument("--min_prom", type = float, default = 15.0, help = "The minimum prominence for peak detection")
parser.add_argument("--min_rbw", type = float, default = 15.0, help = "The minimum reverse bandwidth for peak detection")
parser.add_argument("--max_mean_db", type = float, default = 15.0, help = "The maximum mean power for ruling out a time window")
...
for i, row in harmo_df.iterrows():
    ...
    # Add the mode label
    ax.text(mode_label_x, mode_label_y, f"{mode_order}, {mean_freq:.0f} Hz", 
...
# Set a y axis label for all the axes
i_plot = int(num_plot / 2)
axes[i_plot].text(-0.03, 0.5, "Mode order", fontsize = axis_label_size, transform = axes[i_plot].transAxes, ha = "left", va = "center", fontweight = "bold", rotation = 90)
# Set the axis limits
...
filename = f"jgr_all_modes_freq_vs_time.png"
