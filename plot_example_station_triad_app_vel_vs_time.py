...
fontsize_ax_label = 12.0
fontsize_tick_label = 10.0
fontsize_title = 14.0
...
major_vel_tick_spacing = 1000.0
num_minor_vel_ticks = 5

major_azi_tick_spacing = 90.0
num_minor_azi_ticks = 3
...
for i, triad in enumerate(triads):
    ...
    filename = f"station_triad_app_vels_{mode_name}_{station1_vel}_{station2_vel}_{station3_vel}_mt_win{window_length_mt:.0f}s_min_cohe{min_cohe:.2f}.csv"
    filepath = join(indir, filename)
    ...
    for component in components:
        ...
        # Plot the data
        marker = ax_baz.scatter(app_vel_df["time"], app_vel_df[f"back_azi_{component.lower()}"], 
                        color = color, label = component2label(component), s = markersize_vel, zorder = 2)
    ...
    if i == 0:
        ax_baz.legend(list(legend_dict.values()), list(legend_dict.keys()), loc="upper left", fontsize=fontsize_tick_label, frameon=True, edgecolor="black", facecolor="white", framealpha=1.0)
    ...
    for component in components:
        color = get_geo_component_color(component)
        marker = ax_vel.scatter(app_vel_df["time"], app_vel_df[f"vel_app_{component.lower()}"], 
                        color = color, label = component2label(component), s = markersize_vel, zorder = 2)
    ...
    format_app_vel_ylabels(ax_vel,
                            ...
                            axis_label_size = fontsize_ax_label, tick_label_size = fontsize_tick_label)
    
# Add the super title
mode_order = get_mode_order(mode_name)
fig.suptitle(f"Mode {mode_order:d}, horizontal apparent velocities and back azimuths for example geophone triads", fontsize = fontsize_title, fontweight = "bold", y = 1.04)

### Save the figure ###
figname = f"example_mt_station_triad_app_vels_{mode_name}.png"
